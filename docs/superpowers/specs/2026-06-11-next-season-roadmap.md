...
### EUR-01: Midweek-congestion rotation xmins signal ❌ TESTED & REJECTED 2026-06-15 (exp10)
Built the validation experiment (scope broadened from European-only to midweek congestion = UCL/UEL/UECL + Carabao + FA Cup, per user): hand-researched 2025/26 calendar (`congestion_dates.py`), `(team_id, gw)` clash join (`congestion_join.py`), no-op-default backtest penalty hook, exp10 sweep. **VERDICT: NO_SHIP.** Mechanical gate passed (penalty 0.05: top10 4.753→4.850, clash RMSE 3.226→3.221) but the result is **noise, not signal**: the +2% top10 gain traced to ~7 player boundary-swaps across 7/32 GWs; non-monotonic (peaks at the smallest penalty 0.05, decays as applied harder); the two gates disagree on optimal penalty; permutation test p=0.04 (random same-size clash sets beat it ~5% of the time — fails the strict p≤0.02 robustness bar added to exp10). Confirms the roadmap's LOW prior. Code is spec-compliant + committed (the permutation-gate + calendar live in the repo for any future 2-season re-test); nothing wired live. See rejected-ideas table.

### EUR-01 (original deferral notes)
...
### ODDS-01: Bookmaker-odds clean-sheet / goal-expectation signal ✅ LAB-VALIDATED 2026-06-14 (live wiring gated)
**Result (exp09, 2025/26 archive, leakage-free):** market closing-odds signals BEAT the rolling-goals proxies — VERDICT **SHIP_BOTH**. CS-prob blend (weight 1.0) lifts deploy top10_mean_pts 4.75→5.00; goal-expectation blend (weight 0.25) 4.93→5.04; market CS Brier 0.177. Built: `odds_client.py` (football-data.co.uk closing odds + committed 2025/26 snapshot), `odds_model.py` (de-vig + independent-Poisson supremacy/total → per-team λ; CS=exp(−λ_opp)), `odds_join.py` (team-alias + `(fixture_id, team_id)` lookup, DGW-safe), `backtest.py` no-op-default blend hooks (`odds_cs_weight`/`odds_goalexp_weight`), `exp09_odds.py`. **No live default changed** — this was the validation gate. **Gated follow-up SPEC'D 2026-06-15** → `docs/superpowers/specs/2026-06-15-odds01-live-wiring-design.md` (shadow-first launch posture, TUNE-01-tunable weights, football-data `fixtures.csv` live source, graceful strict=False join). Spec-only — build at 2026/27 launch (live source empty until season); going live = one-line `ODDS_ENABLED` flip after a sanity check. Honest caveat — one season, suggestive not multi-season-proven; re-validate as 2026/27 odds accrue via the in-season tuner. Dixon-Coles low-score correction is the obvious accuracy refinement now that Approach A validated. Original thesis notes below preserved.

### ODDS-01 (original future-experiment notes)
...
| Midweek-congestion rotation xmins penalty (EUR-01) | exp10 2026-06-15: mechanical gate passed but NOT robust — +2% top10 = ~7 boundary-swaps, non-monotonic (peaks at min penalty 0.05), gates disagree on direction, permutation p=0.04 (fails p≤0.02 bar). Noise, not signal. Re-test only with ≥2 seasons. |
