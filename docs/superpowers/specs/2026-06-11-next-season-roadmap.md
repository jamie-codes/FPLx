...
**Result (exp09, 2025/26 archive, leakage-free):** market closing-odds signals BEAT the rolling-goals proxies — VERDICT **SHIP_BOTH**. CS-prob blend (weight 1.0) lifts deploy top10_mean_pts 4.75→5.00; goal-expectation blend (weight 0.25) 4.93→5.04; market CS Brier 0.177. Built: `odds_client.py` (football-data.co.uk closing odds + committed 2025/26 snapshot), `odds_model.py` (de-vig + independent-Poisson supremacy/total → per-team λ; CS=exp(−λ_opp)), `odds_join.py` (team-alias + `(fixture_id, team_id)` lookup, DGW-safe), `backtest.py` no-op-default blend hooks (`odds_cs_weight`/`odds_goalexp_weight`), `exp09_odds.py`. **No live default changed** — this was the validation gate. **Gated follow-up (do next on this thread):** live in-pipeline odds fetch for 2026/27 + run.py wiring + promote the weights; honest caveat — one season, suggestive not multi-season-proven; re-validate as 2026/27 odds accrue. Dixon-Coles low-score correction is the obvious accuracy refinement now that Approach A validated. Original thesis notes below preserved.
