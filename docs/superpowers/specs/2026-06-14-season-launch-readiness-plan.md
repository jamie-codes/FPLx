...
5. ✅ **ML groundwork (§3)** — BUILT 2026-06-15 (ML-01). `pipeline/ml/` (features + position-specific xgboost) + `exp11_ml_shadow.py`. Leakage-free feature extraction (12 as-of signals + fixture/odds/static), train GW7-28 → predict GW29-38, scored vs the formula on the BT-02 metrics. **NO verdict** (in-sample, one season, overstates edge). In-sample result: ML TRAILS the formula on all metrics (top10 4.36 vs 5.18) — the formula is strong; ML edge (if any) awaits the cold 2026/27 cross-season test the harness is built to run. xgboost+scikit-learn pinned; lab-only (no live import). **Build-now agenda COMPLETE.**
