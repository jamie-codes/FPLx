...
describe('computeCaptaincyCandidates', () => {
  it('returns top-5 candidates sorted by projected_captain_pts descending', () => {
    // 7 starting-XI players with different xPts_1gw values
    const projPts = [8.0, 6.5, 5.5, 4.0, 3.5, 3.0, 2.0]
    const allPlayers = projPts.map((pts, i) =>
      makeScoredPlayer({ id: i + 1, xPts_1gw: pts }),
    )
  ...
  it('projected_captain_pts equals xPts_1gw * 2', () => {
    const player = makeScoredPlayer({ id: 1, xPts_1gw: 7.2 })
  ...
  it('only starting-XI picks (position 1-11) considered', () => {
    // Bench player (position 12) has higher xPts_1gw than starting player (position 1)
    const startingPlayer = makeScoredPlayer({ id: 1, xPts_1gw: 5.0 })
    const benchPlayer = makeScoredPlayer({ id: 2, xPts_1gw: 9.0 })
  ...
  it('excludes injured players (mins_risk === "injured")', () => {
    const injuredPlayer = makeScoredPlayer({ id: 1, mins_risk: 'injured', xPts_1gw: 0.0 })
    const healthyPlayer = makeScoredPlayer({ id: 2, xPts_1gw: 5.0 })
  ...
  it('excludes goalkeepers (element_type === 1) even with high xPts_1gw', () => {
    const goalkeeper = makeScoredPlayer({ id: 1, element_type: 1, xPts_1gw: 9.0, mins_risk: 'nailed' })
    const outfielder = makeScoredPlayer({ id: 2, element_type: 4, xPts_1gw: 6.0 })
  ...
  it('excludes players with xPts_1gw <= 0', () => {
    const zeroProjectionPlayer = makeScoredPlayer({ id: 1, xPts_1gw: 0.0, mins_risk: 'nailed' })
    const normalPlayer = makeScoredPlayer({ id: 2, xPts_1gw: 4.0 })
  ...
  it('captain_type is "safe" when mins_risk="nailed" AND gem_score >= position average', () => {
    ...
    const allMids = Array.from({ length: 20 }, (_, i) =>
      makeScoredPlayer({ id: i + 1, element_type: 3, gem_score: 0.50, xPts_1gw: 3.0 }),
    )
  ...
  it('DGW player with high xPts_1gw ranks first', () => {
    const dgwPlayer = makeScoredPlayer({ id: 1, xPts_1gw: 12.0 })
    const normalPlayer = makeScoredPlayer({ id: 2, xPts_1gw: 6.0 })
  ...
  it('returns empty array when no viable candidates', () => {
    // All bench players
    const player1 = makeScoredPlayer({ id: 1, xPts_1gw: 5.0 })
    const player2 = makeScoredPlayer({ id: 2, xPts_1gw: 4.0 })
  ...
  it('returns fewer than 5 when fewer than 5 viable candidates exist', () => {
    // Only 3 starting-XI players with xPts_1gw > 0
    const players = [
      makeScoredPlayer({ id: 1, xPts_1gw: 5.0 }),
      makeScoredPlayer({ id: 2, xPts_1gw: 4.0 }),
      makeScoredPlayer({ id: 3, xPts_1gw: 3.0 }),
    ]
    const squadPicks = [
      makeSquadPick({ element: 1, position: 1 }),
      makeSquadPick({ element: 2, position: 2 }),
  ...
  it('respects custom topN parameter', () => {
    ...
    const players = [
      makeScoredPlayer({ id: 1, xPts_1gw: 8.0 }),
      makeScoredPlayer({ id: 2, xPts_1gw: 6.0 }),
      makeScoredPlayer({ id: 3, xPts_1gw: 5.0 }),
      makeScoredPlayer({ id: 4, xPts_1gw: 4.0 }),
      makeScoredPlayer({ id: 5, xPts_1gw: 3.0 }),
    ]
