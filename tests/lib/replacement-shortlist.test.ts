...
// ---------------------------------------------------------------------------
// Test data setup:
//   sellPlayer: element_type=3 (MID), now_cost=70 (GBP7.0m), xPts_1gw=3.0
...
describe('computeReplacementShortlist', () => {
  it('returns up to 5 same-position alternatives sorted by pts_delta descending', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const squadIds = new Set([1])
    const bankBalance = 15 // GBP1.5m in tenths

    // Create 6 candidates â€” function should return top 5 by pts_delta
    const candidates = [
      makeScoredPlayer({ id: 10, element_type: 3, now_cost: 80, xPts_1gw: 7.0 }), // delta +4.0
      makeScoredPlayer({ id: 11, element_type: 3, now_cost: 80, xPts_1gw: 6.5 }), // delta +3.5
      makeScoredPlayer({ id: 12, element_type: 3, now_cost: 80, xPts_1gw: 6.0 }), // delta +3.0
      makeScoredPlayer({ id: 13, element_type: 3, now_cost: 80, xPts_1gw: 5.5 }), // delta +2.5
      makeScoredPlayer({ id: 14, element_type: 3, now_cost: 80, xPts_1gw: 5.0 }), // delta +2.0
      makeScoredPlayer({ id: 15, element_type: 3, now_cost: 80, xPts_1gw: 4.5 }), // delta +1.5
    ]
  ...
  it('pts_delta equals candidate.xPts_1gw minus sellPlayer.xPts_1gw', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const candidate = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 75, xPts_1gw: 7.0 })
    const squadIds = new Set([1])
  ...
  it('excludes players in squadIds', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const squadMember = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 75, xPts_1gw: 7.0 })
  ...
  it('excludes players with xPts_1gw <= 0', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const zeroPts = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 75, xPts_1gw: 0.0 })
    const negativePts = makeScoredPlayer({ id: 3, element_type: 3, now_cost: 75, xPts_1gw: -1.0 })
    const squadIds = new Set([1])
  ...
  it('only returns players with same element_type as sellPlayer', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const defCandidate = makeScoredPlayer({ id: 2, element_type: 2, now_cost: 75, xPts_1gw: 7.0 })
    const fwdCandidate = makeScoredPlayer({ id: 3, element_type: 4, now_cost: 75, xPts_1gw: 7.0 })
    const midCandidate = makeScoredPlayer({ id: 4, element_type: 3, now_cost: 75, xPts_1gw: 7.0 })
    const squadIds = new Set([1])
  ...
  it('budget_sufficient is true when candidate.now_cost/10 <= bankBalance/10 + sellPlayer.now_cost/10', () => {
    ...
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const affordable = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 80, xPts_1gw: 6.0 })
    const squadIds = new Set([1])
  ...
  it('budget_sufficient is false when candidate exceeds available budget', () => {
    ...
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const tooExpensive = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 90, xPts_1gw: 6.0 })
    const squadIds = new Set([1])
  ...
  it('returns fewer than 5 if fewer candidates qualify', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const candidate1 = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 75, xPts_1gw: 5.0 })
    const candidate2 = makeScoredPlayer({ id: 3, element_type: 3, now_cost: 75, xPts_1gw: 6.0 })
    const squadIds = new Set([1])
  ...
  it('returns empty array when no candidates qualify', () => {
    ...
    const defCandidate = makeScoredPlayer({ id: 2, element_type: 2, now_cost: 75, xPts_1gw: 5.0 })
    const squadIds = new Set([1])
  ...
  it('count parameter limits results', () => {
    ...
    const candidates = Array.from({ length: 8 }, (_, i) =>
      makeScoredPlayer({ id: i + 10, element_type: 3, now_cost: 75, xPts_1gw: 5.0 + i }),
    )
    const allPlayers = [sellPlayer, ...candidates]
  ...
  it('does not include sellPlayer themselves in results', () => {
    const sellPlayer = makeScoredPlayer({ id: 1, element_type: 3, now_cost: 70, xPts_1gw: 3.0 })
    const candidate = makeScoredPlayer({ id: 2, element_type: 3, now_cost: 75, xPts_1gw: 6.0 })
    // squadIds does NOT include sellPlayer (edge case â€” verify id exclusion works without relying on squadIds)
