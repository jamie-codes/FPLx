...
def build_team_def_form_lookup(fixtures: list, window_gws: int = CS_DEF_FORM_WINDOW_GWS) -> dict:
    ...
    # Per team: list of (gw, goals_conceded) in chronological order
    ...
    for fix in finished:
        ...
        team_history.setdefault(h_id, []).append((gw, a_score))  # home team conceded away goals
        team_history.setdefault(a_id, []).append((gw, h_score))  # away team conceded home goals
    ...
    for gw, team_id in gw_team_pairs:
        ...
        prior = [(g, gc) for g, gc in history if g < gw]
        last_n = prior[-window_gws:]
        if not last_n:
            raw_by_gw[gw][team_id] = None  # type: ignore[assignment]  # sentinel for cold start
        else:
            raw_by_gw[gw][team_id] = sum(gc for _, gc in last_n) / len(last_n)
