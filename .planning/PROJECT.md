...
v1.26 Phase 133 complete (2026-05-22) — Price Reset Analysis: `pipeline/price_baseline.py` idempotent write-once capture of `{ str(id): now_cost }` for all 700+ bootstrap elements to Vercel Blob (`price_baseline.json`), gated by `_blob_exists` guard, wired into `run.py` as unconditional non-fatal call (PRST-01). `PriceResetRow`/`ValueTargetRow`/`PriceResetResponse` types in `types.ts`. `GET /api/price-reset` diffs baseline vs current bootstrap, computes Value Targets via position-median xPts filter, returns `{ published, players, value_targets }` (200 always, never 404). `usePriceReset` TanStack Query hook (staleTime 30min). `PriceResetTab` client component: loading/error/published-false empty state ("Prices not yet published" + D-09 copy) / published-true with Price Reset table + Value Targets section; rise pills bg-green + `+` prefix, fall pills bg-red + `−` U+2212 prefix; `#N POS` xPts rank in value-target rows. Registered in `page.tsx` Analyse section between `window` and `price-changes`. 4 pytest + 12 vitest GREEN; PRST-01 PRST-02 PRST-03 PRST-04 verified. Last updated: 2026-05-22
...
v1.24 Phase 126 complete (2026-05-19) — Next Season Planner: `archive_season.py` (GW38-gated concurrent Blob archive, idempotent, 50% guard), `suggest_squad.py` (PuLP ILP 15-player optimal squad fallback), `run.py` GW38 gate. `PreSeasonPlayer`/`PreSeasonSquad`/`SeasonArchiveEntry` types. `buildPreSeasonSquad()` greedy TS builder. `/api/pre-season-squad` (ILP → archive → 404). `usePreSeasonSquad()` (404→null). `NextSeasonPlannerTab` read-only 15-player formation grid + GW1-8 FDR heatmap, graceful empty states. 'next-season' sub-tab after 'rivals' in Plan section. 8 new tests; full suite 1466/1500 GREEN. Last updated: 2026-05-19

v1.24 Phase 125 complete (2026-05-19) — Summer Window Tracker: `SummerWindowTab` (`src/components/news/`) with 5-pill classification filter (All/Confirmed/Rumour/Injury/Rotation), 24h stale banner, article cards with safe external links, `[SKY]`/`[BBC]` source badges, relative dates, empty-state, loading skeleton, error state. `ConfirmedSigningBadge` green pill injected into GemTable expanded rows (mobile + desktop) and OpportunityCostTable buy cluster via `confirmedSigningMap` from TransferPanel. 'window' sub-tab wired into Analyse section between Season and Price Changes. `buildConfirmedSigningMap` shared utility extracted. 32 new tests; full suite 1492 GREEN. Human UAT pending. Last updated: 2026-05-19

v1.24 Phase 124 complete (2026-05-19) — Season Review: `SeasonGwEntry`/`SeasonReview` types, `computeDecisionGrade` (A/B/C/D with D-05/D-06 renormalisation), `GET /api/season-review` route (SSRF guard + parallel FPL fetches), `useSeasonReview` TanStack Query v5 hook (6h staleTime, numeric teamId guard), `SeasonReviewTab` component — empty-state, loading skeleton, REV-01 summary card (6 stats), REV-02 grade card with component scores + methodology note, REV-03 ComposedChart with ChipDot chip-marker renderer, REV-04 'season' SubTab wired into page.tsx. 33 new tests; full suite 1441/1475 GREEN. Last updated: 2026-05-19

v1.24 Phase 123 complete (2026-05-18) — SCRAPER-02 Pipeline: `player_matching.py` shared fuzzy-match utility (rapidfuzz token_sort_ratio ≥85, replaces difflib in lineup_news.py). `transfer_news.py` RSS scraper for Sky Sports + BBC with `TRANSFER_NEWS_ENABLED` env gate, 5-class keyword classifier, per-source isolation, URL dedup, empty-guard. `/api/transfer-news` Route Handler + `useTransferNews()` hook (6h staleTime). `run.py` IS_OFF_SEASON gate wraps 12 GW-dependent steps; transfer_news runs year-round outside gate. 39 new tests + 280 pipeline tests GREEN. 5/5 must-haves verified. Last updated: 2026-05-18
